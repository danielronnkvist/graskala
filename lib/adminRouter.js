...
import { Posts, About, Contact } from './collections.js';
...
import FormContainer from '../imports/ui/admin/contents/formContainer.jsx';
  ...
  action(params) {
    Meteor.call("getAboutData", function(err, data) {
      if (err) {
        ...
        mount(AdminLayout, {
          main: () => <FormContainer data={data} collection={About} />
        });
      }
    });
  }
...
adminRoutes.route('/contact/edit', {
  name: 'Admin.contact.edit',
  action(params) {
    Meteor.call("getContactData", function(err, data) {
      if (err) {
        console.log(data);
      } else {
        if(!data) {
        ...
        mount(AdminLayout, {
          main: () => <FormContainer data={data} collection={Contact} />
        });
      }
    });
  }
